...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., earth_specific_pos=False):
        ...
        if earth_specific_pos:
            self.relative_position_bias_table = nn.Parameter(
                            torch.zeros(window_size[0] * window_size[0] * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
        else:
            self.relative_position_bias_table = nn.Parameter(
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=(7,7), shift_size=(0, 0),
                 ...
                 fused_window_process=False, earth_specific_pos=False):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, earth_specific_pos=earth_specific_pos)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, earth_specific_pos=False):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 norm_layer=norm_layer, earth_specific_pos=earth_specific_pos)
...
class FinalPatchExpand_xP(nn.Module):
    ...
    def __init__(self, input_resolution, dim, dim_scale=4, upscale=4):
        ...
        self.conv_after_body = nn.Sequential(nn.Conv2d(dim, dim, 3, 1, 1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.layer0 = BasicLayer(dim=self.embed_dim, 
        ...
        self.downsample = PatchMerging((patches_resolution[0], patches_resolution[1]), 
                                         dim=self.embed_dim, norm_layer=norm_layer)

        self.layer1 = BasicLayer(dim=self.embed_dim * 2, 
        ...
        self.layer3 = BasicLayer(dim=self.embed_dim * 2, 
                                 input_resolution=(patches_resolution[0] // 2, patches_resolution[1] // 2),
                                depth=self.depths[3],
        ...
        self.upsample = PatchExpand((patches_resolution[0] // 2, patches_resolution[1] // 2), 
                                         dim=self.embed_dim * 2, norm_layer=norm_layer)
        
        self.layer4 = BasicLayer(dim=self.embed_dim, 
                                 input_resolution=(patches_resolution[0], patches_resolution[1]),
                                depth=self.depths[4],
        ...
        self.up = FinalPatchExpand_xP(input_resolution=(self.img_size[0]//self.patch_size,self.img_size[1]//self.patch_size),dim_scale=self.patch_size,dim=self.embed_dim, upscale=self.patch_size)
        self.output = nn.Conv2d(in_channels=self.embed_dim,out_channels=self.out_chans,
...
if __name__ == "__main__":
    ...
    class params_naive:
        def __init__(self):
            ...
            self.window_size = (7, 10) #按照pangu whether的设定，分辨率为(720, 1440)时默认window_size = (6, 12)
            self.mlp_ratio = 4.
            self.qkv_bias = True
            self.qk_scale = None
            self.ape = False
    ...
    sample = torch.randn(1, params.N_in_channels, params.img_size[0], params.img_size[1])
